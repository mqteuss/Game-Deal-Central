...
export default function App() {
  ...
  const [exchangeRateWarning, setExchangeRateWarning] = useState<string | null>(null);
  ...
  const toggleMonitor = async (deal: GameDeal) => {
    ...
    if (!supabase) {
      toast('Supabase não está configurado', 'error');
      return;
    }
    ...
    if (isMonitored) {
      const previousGames = monitoredGames;
      ...
      if (error) {
        ...
        setMonitoredGames(previousGames);
      } else {
    ...
    const fetchInitialData = async () => {
      try {
        const [fetchedStores, rateRes] = await Promise.all([
          ...
          fetch('https://economia.awesomeapi.com.br/last/USD-BRL', { signal: controller.signal }).catch(() => null)
        ]);
        ...
        if (rateRes && rateRes.ok) {
          ...
          const parsedRate = parseFloat(rateData.USDBRL?.ask);
          if (Number.isFinite(parsedRate) && parsedRate > 0) {
            setExchangeRate(parsedRate);
            setExchangeRateWarning(null);
          } else {
            setExchangeRate(5.0);
            setExchangeRateWarning('Cotação USD-BRL indisponível. Os preços estão usando uma estimativa temporária.');
          }
        } else {
          setExchangeRate(5.0);
          setExchangeRateWarning('Cotação USD-BRL indisponível. Os preços estão usando uma estimativa temporária.');
  ...
  useEffect(() => {
    ...
    const fetchDeals = async () => {
      try {
        ...
        const params: GetDealsParams = { onSale: true, pageSize: 60, pageNumber, sortBy };
        
        if (debouncedSearch) params.title = debouncedSearch;
        if (selectedStores.length > 0) params.storeID = selectedStores.join(',');

        const minPriceBRL = debouncedMinPrice ? parseFloat(debouncedMinPrice) : undefined;
        const maxPriceBRL = debouncedMaxPrice ? parseFloat(debouncedMaxPrice) : undefined;
        
        if (minPriceBRL !== undefined && (!Number.isFinite(minPriceBRL) || minPriceBRL < 0)) {
          setError('Preço mínimo inválido.');
          setHasMore(false);
          return;
        }
        if (maxPriceBRL !== undefined && (!Number.isFinite(maxPriceBRL) || maxPriceBRL < 0)) {
          setError('Preço máximo inválido.');
          setHasMore(false);
          return;
        }
        if (minPriceBRL !== undefined && maxPriceBRL !== undefined && minPriceBRL > maxPriceBRL) {
          setError('O preço mínimo não pode ser maior que o preço máximo.');
          setHasMore(false);
          return;
        }

        if (minPriceBRL !== undefined) {
          params.lowerPrice = Math.max(0, minPriceBRL / exchangeRate);
        }
        if (maxPriceBRL !== undefined) {
          params.upperPrice = maxPriceBRL / exchangeRate;
        }
        if (minMetacritic > 0) params.metacritic = minMetacritic;
        if (minSteamRating > 0) params.steamRating = minSteamRating;

        const nextDeals = await getDeals(params, controller.signal);
  ...
  const storeById = useMemo(
    () => new Map(availableStores.map(store => [store.id, store])),
    [availableStores],
  );
  ...
  const offersDeals = useMemo(() => 
    deals.map(deal => {
      const storeObj = storeById.get(deal.storeID);
      ...
      <div className="flex pt-28 min-h-screen">
        ...
        <main className="flex-1 p-4 md:p-8 md:ml-64">
          <div className="max-w-7xl mx-auto">
            ...
            {exchangeRateWarning && !showMonitoredOnly && (
                  ...
                  {offersDealsFiltered.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 text-center px-4">
                      ...
                      <h3 className="text-2xl font-bold text-white mb-3">
                        {hasMore && minDiscount > 0 ? 'Buscando ofertas compatíveis' : 'Nenhum jogo encontrado'}
                      </h3>
                      <p className="text-zinc-400 max-w-md text-lg leading-relaxed">
                        Não encontramos nenhuma oferta que corresponda aos seus filtros atuais. Que tal tentar uma busca diferente?
                      </p>
                      {hasMore && minDiscount > 0 && (
                        <div ref={offersObserverRef} className="h-8 w-full" />
                      )}
                      <button 
                        onClick={() => {
                          setSearchQuery('');
