...
export const AuthModal = ({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!supabase) {
        setError('Supabase não está configurado. Verifique as variáveis públicas do projeto.');
        setIsLoading(false);
        return;
      }
      ...
      if (mode === 'register') {
        ...
        if (signUpData.session && signUpData.user) {
          ...
          const pendingProfileKey = `pending_profile:${normalizedEmail}`;
          if (avatarFile) {
            const reader = new FileReader();
            reader.onloadend = () => {
        ...
        if (pendingProfile && signInData.user) {
          try {
            const parsed = JSON.parse(pendingProfile) as { username?: string; avatar?: string | null };
            if (parsed.username) {
              await saveProfileData(signInData.user.id, parsed.username, safeEmail, parsed.avatar);
            }
          } finally {
          ...
          <div className="p-6">
            {!isSupabaseConfigured && (
              <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-sm">
                Supabase não está configurado. Configure `VITE_SUPABASE_URL` e `VITE_SUPABASE_PUBLISHABLE_KEY`.
              </div>
            )}
