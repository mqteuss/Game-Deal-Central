...
export const GameModal: React.FC<GameModalProps> = ({ game, onClose, exchangeRate, availableStores }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        setIsLoading(true);
        const data = await getGameDetails(game.gameID, controller.signal);
        setDetails(data);
      } catch (err) {
  ...
  const currentPrices = details?.deals.map(deal => parseFloat(deal.price)).filter(Number.isFinite) ?? [];
  const cheapestCurrentPrice = currentPrices.length ? Math.min(...currentPrices) : null;

  const isAtHistoricLow = details && cheapestCurrentPrice !== null
    ? parseFloat(details.cheapestPriceEver.price) >= cheapestCurrentPrice
    : false;
          ...
          <div className="relative h-48 sm:h-64 w-full bg-black/50 flex-shrink-0">
              ...
              <div className="relative" ref={shareRef}>
                ...
                {showShare && (
                  <div className="absolute z-20 top-full mt-2 right-0 bg-zinc-800 border border-white/10 rounded-lg shadow-xl shadow-black/40 py-1 min-w-[160px]">
                    ...
                    <button onClick={() => { window.open(`https://wa.me/?text=${encodeURIComponent(shareText)}`, '_blank', 'noopener,noreferrer'); setShowShare(false); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-zinc-300 hover:bg-white/5 hover:text-white transition-colors">
                      <MessageCircle size={14} />
                      WhatsApp
                    </button>
                    <button onClick={() => { window.open(`https://t.me/share/url?url=${encodeURIComponent(game.url)}&text=${encodeURIComponent(`🎮 ${game.title} — ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(game.discountedPrice)} (-${game.discountPercentage}%)`)}`, '_blank', 'noopener,noreferrer'); setShowShare(false); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-zinc-300 hover:bg-white/5 hover:text-white transition-colors">
                      <Send size={14} />
                      Telegram
                    </button>
                  </div>
            ...
            <div className="absolute bottom-0 left-0 p-6 w-full">
              <div className="flex items-center gap-3 mb-2">
                <h2 id="game-modal-title" className="text-2xl sm:text-3xl font-bold text-white leading-tight">{game.title}</h2>
